...
The verifier applied one identity to every file -- stored gain must equal `--target-lufs` minus the fresh measurement -- but Opus R128 keys are -23-LUFS-referenced by the Opus/R128 spec, so every correctly-written R128 album reported `off by exactly -5.00 dB` at the -18 default. The reader now surfaces which key family each file's gains live in (`read_replaygain_values_with_convention`, joining `read_replaygain_values`, which keeps its shape): `replaygain_*` files verify at `--target-lufs`, R128 files at the -23 baseline their format implies, and the report header counts both. A genuinely wrong R128 gain still flags, with the -23-based expectation in the row. Found by B.7's first real-library run (the decision-#110 gate): the gains in the reference slice were internally consistent per write reference (MP3s at -18, opus at -23), so the held health-score bucket stays closed -- and the run closed the nested-parent direct-file scan option too (zero nested albums).

Also: `justfile` (test/lint/fmt/version -- `PYTHONPATH=src` is baked in, closing the stale-site-packages trap for good), the dated research reports archived under `docs/research/`, and the research-section roadmap boxes ticked with their ship records.

## v5.5.0 (2026-09-14)
- **New mode: `--verifyReplayGain`** (B.7, the recorded next lane): the audit that checks whether stored ReplayGain values are RIGHT rather than merely present. The engine is rsgain itself, scan-only (`rsgain custom -l N -a -O -q -s s`): the fresh measurement comes from the same libebur128 that computed the stored tags, so stored == target - measured holds exactly and a delta beyond `--tolerance` (default 0.5 dB) means a wrong or mismatched tag, not jitter. One call per album folder with the album aggregate for the album-gain check; clip-protected rows are reported separately, never flagged OFF (clip protection legitimately breaks the identity). Knobs: `--target-lufs` (default -18; a -14-targeted library verifies clean only at -14) and `--tolerance`. Requires rsgain on PATH; refuses with exit 2 without it, like the integrity modes' missing-decoder refusal. The tags layer grows `read_replaygain_values`, a key-aware value reader (standard replaygain_* strings as dB, Opus R128 integers as Q7.8 /256, MP4 freeform bytes decoded), and scripts/replaygain.py's `read_gain_strings` now delegates to it and formats, so the two readers cannot drift (log strings unchanged for dB-convention tags). Report sections: GAIN OFF (with the numbers), UNGAUGED, CLIP-ADJUSTED, SCAN FAILED, and OK with `--verbose`. Wired into the CLI and the TUI's Metadata section.
- **New mode: `--checkPlaylists`** (B.4): `--playlist` writes absolute-path .m3us that every mover the repo ships can orphan; this walks the roots for .m3u/.m3u8 files and reports, per playlist, entries whose target no longer exists (with line numbers) and a missing #EXTM3U header. Relative entries resolve against the playlist's own directory, so hand-made playlists check the same as generated ones. `--verbose` also lists the clean ones.
- **New mode: `--auditAlbums`** (B.5): per-album consistency across three lenses: mixed codecs (the leftover .mp3 beside the .flac), track-number gaps over 1..max plus duplicates and untagged files, and year tags (missing, partial, or diverging across files). The year read is additive on `TagBundle` (a new trailing field) via a generic late pass in get_all_tags: year keys share naming, not frame machinery, so suffix matching (TDRC/TYER, Vorbis date keys, MP4 day atom, ASF WM/Year) covers every container; MusicBrainz keys are skipped (their "...Date" TXXX descs carry MBIDs).
- **New: `--snapshot` / `--diff`** (B.10): the before/after evidence half for every writer. `--snapshot` writes a per-file TSV (path, size, mtime, track/rating/genre/year, ReplayGain presence); `--diff` replays one against the current tree: MOVED (paired on size+mtime, the signature mv-only writers preserve), RETAGGED (per-field old -> new), RESIZED, ADDED, REMOVED. Compound runs compose because the diff is state-versus-state, never log replay. A file without the snapshot header is refused (exit 2), never diffed as everything-removed.
- **New mode: `--auditArtMismatch`** (B.9): compares the extractArt pick (same priority-ordered embedded source) against the folder cover. MATCH is byte-identical; SAME PIXELS is a different-bytes re-encode at identical dimensions (benign); DIFFERENT IMAGE is a real mismatch, reported with both dimensions. Folders carrying only one of the two are not findings: players resolve a missing half silently, but there is nothing to disagree about.
- **New mode: `--auditJunkFrames`** (B.8): audits MP3 ID3v2 tags for obsolete frames (v2.3 leftovers like TYER/TSIZ, plus v2.2's three-letter names), empty text frames, and nonstandard iTunes-era frames (TCMP, GP1, TGID...), which are reported so keeping them is a choice, never called junk. The audit loads the tag raw (`translate=False`) because mutagen's default load silently upgrades TYER to TDRC, hiding exactly the junk being audited. Duplicate frames are documented as undetectable here: mutagen's dict model collapses same-hash frames at parse. MP3-only; stripping stays with apestrip and the write modes (the same audit/strip split as the ReplayGain pair).
- **Fixed (HIGH, latent since v4.17.0): the TUI library submenu's migration regressions**, one root cause: the c6575dc vir-tui migration rebuilt the submenu without the kwargs the mode functions had grown, and the error containment paged the TypeError politely instead of surfacing it. "Generate smart playlist (.m3u)" called `generate_playlist` without the required rule and died on every invocation; the rule and layout prompts are back. The layout kwarg and the genre/paths toggles are restored on all four library entries (a genre-first config's exports were mis-attributed). Multi-root configs scan all roots again, exactly as the CLI passes them, with the root count in the title; the write modes refuse a multi-root config like the CLI instead of silently picking the first tree; the TUI's AI-export default realigns to the CLI's library_ai.txt; the health-score entry gains the CLI's --verbose toggle. A new kwargs-parity suite drives both faces with the modes patched out and compares each call bound to the mode's signature, so any future cli/tui drift fails the suite instead of paging a TypeError (13 new wiring tests).
- **Fixed: integrity modes tell the truth about what they verified.** `--testFLAC` now honors `--ffmpeg` (the path never reached run_flac_mode) and warns when an explicit `--prefer ffmpeg` cannot be honored instead of silently switching decoders. A decode scan with no decoder refuses with exit 2: before, a typo'd `--ffmpeg` path graded every file OK ("decode check skipped") with exit 0. FLAC `--resume` folds cached records in with .get like the decode path, so a state record damaged by the interrupt degrades to OK instead of crashing the resumed scan.
- **CI/publish hardening:** publish.yml's three actions are SHA-pinned (checkout v4.4.0, setup-python v5.6.0, pypi-publish v1.14.2) with a new assertion step that refuses to build unless the tag equals both pyproject.toml's version and config.VERSION (the v5.2.0 incident class); workflow permissions drop to least-privilege with a publish concurrency group; ci.yml tests the tree (PYTHONPATH=src) instead of the pip-installed copy and SHA-pins its actions too; the ruff==0.15.14 pin moves into the dev group so local format output matches CI; actions-only dependabot keeps the pins current.
- **GitHub:** SECURITY.md; a release.yml opens the GitHub Release automatically on every future tag (body = the verbatim tag message, stock gh CLI; per Brandon's decision, closing the gap that left v5.0.0-v5.4.0 without Releases until the 09-13 hand batch); CI and PyPI badges join the README badge row.
- **Docs:** the hero screenshot is recaptured from the current TUI (it still showed the v4.10.2 pre-rename menu, then was recaptured again after the feature lanes so all six new modes render); vir-tui is enumerated where the other runtime deps are (README Requirements and Credits, spec's binary section, which had listed two of three); the lattice-import skill's step 2 no longer passes `-y` to apestrip (argparse error since the 5.0.0 fold) and follows the lattice-music rename; nine stale comments corrected (the TUI's deliberate lattice_ output-name prefix stated, the tag-workers consumer list completed with the GIL-vs-hashing caveat, flac2opus's docstring no longer claims metadata verification, and so on); four doc truth riders (README's multi-root exception for the write modes, --only-errors covering WAV/WMA, spec's TUI sections, spec §5's write-set wording); README's companion table and architecture list completed; the stale kernel microversion dropped.
- **Dependency:** the vir-tui floor moves to `>=2.5.0` (landed 2026-09-14 after v5.4.0; upstream 2.4.0/2.5.0 are additive terminal-safety and session-awareness releases, and the suite has been running against them since).
- **Tests:** 74 new (wiring/integrity plus the six new modes'): 560 to 634, green; ruff@0.15.14 clean. Two tests had hidden dependencies on the dev machine's environment and now pass anywhere: the MP3 resume test stubs its decoder (the new missing-decoder refusal used to fire first on a runner without ffmpeg) and resolves ffmpeg identically across both runs (the resume scan_id carries the resolved path, so a stubbed run could not resume against a PATH-resolved one); the --verifyReplayGain run tests stub rsgain's PATH lookup the same way.

## v5.4.0 (2026-09-13)
- **New mode: `--auditAudioDupes`** (the content-hash duplicate audit from the 2026-09-12 landscape research; closes the retagged/renamed blind spot in the tag/name/size-based `--duplicates`). One read-only pass, tags never consulted (it runs even without mutagen), matching on three sha256 tiers: **exact** (the whole file byte for byte; catches renamed and copied dupes), **audio-stream** (the container's tag regions are parsed and skipped, so MP3s differing only in ID3v2/ID3v1/APEv2, FLACs differing only in metadata blocks, and M4As differing only in box layout match on identical audio), and **sampled** (raw first/last 64KB fingerprints for the formats without a parsed region; survives a rename and most retags). Malformed containers fall back safely; nothing is ever matched across encodings. Writes `audio_dupes.txt` by default; wired into the CLI and the TUI's Metadata section.
- **New mode: `--healthScore`** (the library health score from the same research): one read-only pass scores every album out of 100, aggregating the existing audit lenses: tag completeness (40 points), ReplayGain coverage (30), art presence and resolution (20), and the bitrate floor (10). Albums below full score are listed worst-first with point-by-point deductions and letter grades (A/B/C/D); `--verbose` also lists the perfect albums. No decode scans: the integrity walks remain their own modes. Writes `health_score.txt` by default; reuses the `--min-bitrate` and `--min-art-res` floors; wired into the CLI and the TUI's Metadata section.
- **Docs:** the README's "completed software" claim (falsified by the v5 line itself) is reworded to maintained software with the audit family still accruing; the pre-fold descriptions in pyproject.toml and `--help` are refreshed (pyproject feeds PyPI); CLAUDE.md's progress-bar dispatch line now names `vir_tui.progress_box`, the real implementation, instead of the `_TUIPbar` deleted in v4.17.0. The README's sample-output em-dashes stay: they mirror the library tree's real join, which prints a literal em-dash between artist and title (verified against `library.py`).
- **Tests:** 43 new: synthetic duplicate fixtures for all three matching tiers (identical, retagged, distinct), unit coverage for the ID3v2/ID3v1/APEv2/FLAC/MP4 boundary parsing and the trailing-tag strip, health-scoring units for every deduction bucket and the grade bands, patched-reader run tests for both modes, and the CLI/TUI wiring pins. Suite 517 to 560, green; ruff@0.15.14 clean.
