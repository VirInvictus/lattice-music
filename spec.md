...
| ReplayGain verification | `--verifyReplayGain` | Re-measure each album read-only via `rsgain custom -O -s s` (the writer's own engine) and report stored gains disagreeing with the fresh measurement by more than `--tolerance` at the reference each file's key family implies (replaygain_* keys at the assumed `--target-lufs`; Opus R128 keys at the R128 -23 LUFS baseline); clip-adjusted rows are reported, never flagged OFF; requires `rsgain` |
